...
def lowLevelStrategy(self, endx, endy):
    ...
    for key in game.enemies:
        enemy = game.enemies.get(key)

        if enemy.state == "ACTIVE" and game.weightedMap[self.me.y][self.me.x] == int(self.maxWeight / 2):
            if self.me.x == enemy.x:
                self.chatSocket.sendInChat(game.name, enemy.name + " I kill u bastard!!! RATATATRATATATAAAAAAA")
                if self.me.y > enemy.y:
                    nextActions.append(("shoot", "N"))
                else:
                    nextActions.append(("shoot", "S"))
            elif self.me.y == enemy.y:
                self.chatSocket.sendInChat(game.name, enemy.name + " I kill u bastard!!! RATATRATATATATAAAAAAA")
    ...
    if direction == "E" and game.weightedMap[self.me.y][self.me.x + 1] == int(self.maxWeight / 2):

        # my x becomes  x+1
        if game.serverMap[self.me.y][self.me.x + 1] == "~":
    ...
    elif direction == "W" and game.weightedMap[self.me.y][self.me.x - 1] == int(self.maxWeight / 2):
    ...
    elif direction == "S" and game.weightedMap[self.me.y + 1][self.me.x] == int(self.maxWeight / 2):

        # my y becomes  y+1
        if game.serverMap[self.me.y + 1][self.me.x] == "~":
            nextActions.append(("move", direction))
        else:
            for key in game.enemies.keys():
                enemy = game.enemies.get(key)
    ...
    elif direction == "N" and game.weightedMap[self.me.y - 1][self.me.x] == int(self.maxWeight / 2):
