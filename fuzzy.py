...
def FuzzyControlSystem():
    ...
    # antecedent visualization example
    # energy.view()

    # poor mediocre average decent good
    needEnergy = ctrl.Rule(((energy['poor']) & (d_flag['average'] | d_flag['good']) & (num_enemies['good'] | num_enemies['average']) & d_recharge['poor'])
                      , output['goToRecharge'])
    ...
    ctf = ctrl.Rule((d_flag['poor']) &
                      (energy['good'] | energy['average']) & (d_recharge['average'] | d_recharge['good']) & (d_safeZone['average'] | d_safeZone['good'])
                      , output['goToFlag'])
    ...
    sim.input['energy'] = 256
    sim.input['d_flag'] = 32
    sim.input['d_recharge'] = 30
    sim.input['num_enemies'] = 0
